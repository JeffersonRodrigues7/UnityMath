...
public class Ex10_MouseLook : MonoBehaviour
{
    ...
    void UpdateMouseLook()
    {
        ...
        pitchDeg += -yDelta * mouseSensitivity;//Vertical mouse movemente rotaciona ao longo do eixo X (Pitch), o sinal de menos é para inverter a direção
        ...
        transform.rotation = Quaternion.Euler(pitchDeg, yawDeg, 0);//Fazendo rotação em si
    }
    ...
    void PlaceTurret()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit))
        {
            ...
            //Primeiro faz a rotação Quaternion.LookRotation(zAxis, yAxis) e depois offsetRot, a ordem aqui é importante, precisa ser no local space no caso
